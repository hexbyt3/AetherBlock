/**
 * @file net_test.c
 * @brief Network connectivity tester for blocked hostnames.
 *
 * Attempts a non-blocking TCP connect to each hostname on port 443 to
 * verify whether the DNS block is effective. Results are gathered one
 * entry per frame so the UI stays responsive.
 */
...
/**
 * @brief Prepare a network test run from the current hosts file entries.
 * @param ts  NetTestState to initialize.
 * @param hf  HostsFile whose non-comment entries will be tested.
 */
...
/**
 * @brief Test whether a single hostname is reachable via TCP on port 443.
 * @param hostname        Hostname to resolve and connect to.
 * @param out_latency_ms  If non-NULL, receives the connection time in ms (or -1).
 * @return true if the host is reachable, false if blocked or unreachable.
 */
...
/**
 * @brief Execute one test step, testing the next pending hostname.
 * @param ts  NetTestState to advance. Marks itself finished when all entries are tested.
 */
