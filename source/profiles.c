/**
 * @file profiles.c
 * @brief Preset blocking profiles and default server catalog.
 *
 * Defines the built-in catalog of known Nintendo servers and provides
 * functions to seed defaults and apply preset blocking profiles
 * (block all, allow game updates, telemetry only, allow all).
 */
...
/**
 * @brief Add all catalog entries to the hosts file if not already present.
 * @param hf  HostsFile to seed with default entries.
 */
...
/**
 * @brief Apply a preset profile by enabling/disabling entries by category.
 * @param hf      HostsFile whose entries will be modified.
 * @param preset  The profile preset to apply.
 */
void profileApply(HostsFile *hf, ProfilePreset preset) {
    /* First ensure all catalog entries exist */
